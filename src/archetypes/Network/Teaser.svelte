<script>
  ...
  const handleUpdate = props => {
    ...
    deployingCount =
      ...
      _.filter(props.nodes, { status: "CONFIGURING" }).length;
    ...
    // } else {
    //   Chainspec.query(fetchOneChainspec, { _id: props.chainspec }).then(
    //     _chainspec => {
  ...
  :global(.network-teaser.smui-paper) {
    * :global(.badge) {
    ...
    &[data-status="DEPLOYING"] {
      .title {
        :global(.smui-paper__content > .status) {
...
<Hotwire
  subscriptions={[{ name: _id, event: `NODESTATUS`, callback: ({ nodes }) => {
      ...
      } }, { name: _id, event: `UPDATE`, callback: handleUpdate }]}>
  <Paper
    class="network-teaser"
    on:click={() => OpenDrawer(NetworkDetail, { _id: _id })}
    elevation="0"
    data-status={status}>
    <div class="title">
      <Title>
            ...
            <Item
              on:click={e => {
                ...
                dialog.warning({
                  ...
                  subtitle:
                    'Deleting this network will remove the network and all nodes. This cannot be undone',
                  confirmButton: 'Confirm Network Deletion',
                  onConfirm: () =>
                    Network.query(deleteOne, { _id: _id }).then(() =>
